...
| `meta.json`                          | `services/page.js`   | Object keyed by slug: `{ title, description, type, access, owner, createdAt, updated, tags }` per page. `tags` is an array of up to 3 PascalCase strings; missing means `[]`. |
...
| `tags` + `page_tags` | `services/tags-store.js` | `migrations/004-tags.sql`       | Tag index with maintained `public_count`/`publisher_count` per tag; `page_tags` is the slug↔tag join carrying each page's access. |
...
-- tags + page_tags (migration 004)
CREATE TABLE tags (
  tag_key         TEXT PRIMARY KEY,       -- lower(name); the case-insensitive match/dedup key
  name            TEXT NOT NULL,          -- canonical display casing, e.g. "MachineLearning"
...
CREATE TABLE page_tags (
  ...
  access      TEXT NOT NULL DEFAULT 'publisher',   -- which count bucket this row feeds
  created_at  TIMESTAMPTZ NOT NULL DEFAULT now(),
  PRIMARY KEY (slug, tag_key)
);
-- A trigger on page_tags keeps tags.{public,publisher}_count in sync and prunes
-- tags whose counts both reach zero. See migration 004 for the trigger body.
```
...
**`meta.json` is still the source of truth for a page's tags; `tags`/`page_tags` is a derived index.** The array in `meta.json` says which tags a page has. The Postgres tables exist only to answer "order all tags by document count" without scanning every page — a query S3 JSON can't do cheaply. `tags-store.setTagsForPage(slug, tags, access)` re-syncs the index after any publish, tag edit, or access change, so the two never drift. Because it's only an index, the tag features degrade gracefully when `DATABASE_URL` is unset: writes become no-ops and `GET /api/tags` falls back to `tagsService.countTags(visiblePages(...))`, deriving counts from the page list.

**Counts are split by access so the public view can't leak gated pages.** `public_count` tallies pages with `access = 'public'`; `publisher_count` tallies `access = 'publisher'`. An anonymous viewer reads `public_count` only; an admin reads the sum. The trigger keys off `page_tags.access` to pick the bucket, which is why an access change has to go back through `setTagsForPage` (it deletes and re-inserts the join rows so the trigger moves the count to the right bucket).
...
- The *grouping* of pages (tags) → `tags` array in `meta.json` (truth) + `tags`/`page_tags` in Postgres (count index)
